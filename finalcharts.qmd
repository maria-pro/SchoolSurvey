...
cols_engA <- c("#065f24","#6ee696")

cols_engB <- c("#fa492c","#bf4c3a", "#fd8773")
...
scale_fill_manual(values=c('#4e81bc','#e32f2a', '#7da52b', '#848484'))+
  ...
  scale_y_discrete(limits=c(0, 25), breaks=NULL, expand = expansion(mult = 0.5))+
...
#    scale_x_discrete(labels = function(x) str_wrap(x, width = 10))+
  ...
  scale_y_continuous(limits = c(0, 1.1), breaks = seq(0, 1, 0.20), labels = scales::percent)+
    ...
    geom_text(aes(label = round(Mean*100), group = fct_rev(engB), fill=fct_rev(engB)), 
        ...
        hjust = - 0.2,
        size=2.75)+
  scale_fill_manual(values=cols_engB)
...
axis.line.x = element_line(colour = "black"))+
  ...
  scale_y_continuous(limits = c(0, 100), breaks = seq(0, 100, 20), expand = expansion(mult = 0.5))+
geom_text(aes(label = paste0(round(Mean*100),"%")), 
